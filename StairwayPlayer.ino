...
#define button_play1 10 //кнопка замыкатель 1
#define button_play2 11 //кнопка замыкатель 2
#define enable_play_led 13 //светодиод активации воспроизведения

short volume; //громкость
short track_num; //номер трэка
  ...
  if (millis() - last_time > 1000)
  {
    ...
    if (enable_play) {
      ...
      if ((digitalRead(button_play1) == ACTIVATED && playing_now == false) || (digitalRead(button_play2) == ACTIVATED && playing_now == false))
      {
        ...
        //Serial.println( digitalRead(button_play1));
        //Serial.println( digitalRead(button_play2));
      }
      ...
      execute_CMD(0x16, 0, 0);
      Serial.println( "play stop disable");
    }
  }
}
